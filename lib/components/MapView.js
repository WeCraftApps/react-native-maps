...
const propTypes = {
  ...
  /**
   * Image that is fixed to the map
   *
   * The position defines the center of the image (50% from the top of the
   * image and 50% from the left)
   */
   groundOverlay: PropTypes.shape({
       /**
        * Image used for the overlay
        */
       ...
       */
        ...
        */
        ...
        /**
         * Amount that the image should be rotated
         */
        bearing: PropTypes.number,

        /**
         * Transparency applied to the image in the range [0...1], where 0
         * means the image is opaque and 1 the image is fully transparent
         */
         transparency: PropTypes.number,
   }),

   /**
    * Coordinates defining boundaries where the user can drag the map
    *
    * The boundaries are defined by indicating the northeast point and the southwest point
    */
   scrollableBoundaries: PropTypes.shape({
       /**
        * Coordinates of the northeast point of the scrollable area
        */
       ...
       /**
        * Coordinates of the southwest point of the scrollable area
        */
  ...
  render() {
    ...
    if (this.state.isReady) {
      props = {
        ...
        groundOverlay: {
            ...this.props.groundOverlay,
            image: image
        },
      };

      let image;
      if (typeof(this.props.groundOverlay) !== 'undefined' && this.props.groundOverlay.image) {
        image = resolveAssetSource(this.props.groundOverlay.image) || {};
        image = image.uri;

        if(!__DEV__) {
            const assets = AssetRegistry.getAssetByID(this.props.groundOverlay.image);

            image = `http://localhost:8081${assets.httpServerLocation}/${assets.name}.${assets.type}?platform=${Platform.OS}&hash=${assets.hash}`;
        }
